...
from okf_core.parser import FORM_ABSOLUTE, FORM_EXTERNAL, FORM_RELATIVE, parse
...
def test_crlf():
    ...
    assert [(l.target, l.form) for l in doc.links] == [("/other.md", FORM_ABSOLUTE)]
...
def test_bom():
    ...
    assert [(l.target, l.form) for l in doc.links] == [("sibling.md", FORM_RELATIVE)]
...
def test_yaml_error_reports_fm_error_with_body():
    ...
    assert [l.target for l in doc.links] == ["/v.md"]
...
def test_appendix_a_all_files_parse_without_exception():
    ...
    assert {l.target for l in sales.links} == {"/tables/orders.md", "/tables/customers.md"}
    orders = parse(bundle / "tables" / "orders.md")
    forms = {l.target: l.form for l in orders.links}
...
def test_bytes_input_and_line_numbers():
    doc = parse(b"---\ntype: t\n---\nline4 [a](/x.md)\nline5 [b](y.md)\n")
    assert [(l.line, l.form) for l in doc.links] == [(4, FORM_ABSOLUTE), (5, FORM_RELATIVE)]
