...
접지 린트(layer-aware, 플러그인측)가 아래를 **warn**으로 신호한다. OKF §9 컨포먼스
error로 승격하지 않는다(스펙 관용 — 미지·미기재 필드 거부 금지).

1. **정초 순서** — `derived_from` 대상은 **출처 개념보다 엄격히 낮은 층**이어야 한다
   (지혜→{지식,정보}, 지식→{정보}). 상위·역방향 파생(예: 정보가 지혜에서 파생)은 사다리
   위반이다.
2. **접지(출처)** — 정보 층은 `resource` 또는 `# Citations`를 가져야 한다. 상위 층
   (지식·지혜)은 비어있지 않은 `derived_from`을 가져야 한다(떠 있는 판단 방지).
3. **대상 존재** — `derived_from` 경로가 번들에 없으면 warn한다(§5.3 "미작성 지식 신호").
