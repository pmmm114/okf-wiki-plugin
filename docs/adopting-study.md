...
- **격리 요건(URL vault)** — 관리형 clone은 유저 스코프 단일 자원이라, 핸들러가 그
  체크아웃 브랜치를 바꾸거나 미커밋 잔재를 남기면 이후 신선도 갱신(ff)이 막힌다.
  URL vault용 핸들러는 **`git worktree`로 임시 워크트리**를 만들어 거기서 브랜치·커밋·push
...
### 지식 Vault repo 패턴 (#114)

vault는 **순수 지식 목적지**다 — 큐레이션된 지식만 담고 런타임 스테이징을 담지 않는다.

| 요소 | 규약 |
| --- | --- |
| 구조 | `.okf/` 큐레이션 번들(index·log·개념, strict-valid) + `.okf-wiki.json`(vault 지목 설정, `study.capture`). **런타임(inbox/ledger/trust)은 vault에 없다** |
| 런타임 위치 | `~/.claude/okf/study`(유저 스코프) — 스테이징은 vault repo가 아니라 여기에 쌓인다 |
| 역할 | 승격 대상 — `/study`가 후보를 검수해 `.okf/`에 큐레이션 편집만 쓴다(git diff로 확인·커밋) |
| 검증 | `okf validate .okf --strict`(번들 건강) + `/okf-doctor`(vault 부합·스코프 트레이스) |

→ vault는 스캐폴드·조작 대상이 아니다. **vault 안에서 세션을 열 필요가 없고**(그러면
...
vault 값은 **로컬 clone 절대경로** 또는 **repo URL**(ssh/https/git/file, #153) 둘 다 된다.
vault repo는 `.okf/`가 이미 있는 지식 repo면 된다. `study.capture`가 꺼져 있으면
마법사가 켜기를 제안한다 — vault `.okf-wiki.json`의 설정만 켜고 런타임은 유저
스코프에 둔다(vault엔 `.okf-study`를 만들지 않는다). `review` 권장(`auto`는 세션
시작 넛지가 모든 무설정 세션에 따라온다).

### URL Vault — 관리형 clone (#153)
...
승격은 언제나 vault `.okf/`로 간다(위 스테이징 → `/study` 검수 → vault 번들). 자동 캡처의
스코프는 위치가 정하고(이벤트당 정확히 하나), 의도가 있을 때만 `/study --scope
vault|project`로 벽을 넘는다.
...
- 지식·이력의 **정본은 번들 + log.md + git**이다. 스테이징(후보 큐·원장·이벤트 저널)은
  **단일 SQLite `study.db`**(#130)에 담기는 소모성 런타임 상태다 — 드레인되면 소모된다.
  ...
  vault `.okf/log.md`에 새겨 **버저닝을 git에 남긴다**(#114 U5 · #132).
...
- 포인터가 깨진 기간의 미큐잉은 `study scan` → `study scan --enqueue`(멱등)로
  회복한다. 막히면 `/okf-doctor`가 스코프 해소·vault 부합·스토어 건강(`_sqlite3` 유무·
...
> 구현: Epic #91(폴백·마법사·전역 원장·doctor) + #114(런타임 유저 스코프 분리·vault
...
| vault 폴백(선택) | `/okf-init --vault <path>` → 어디서든 적립(§7) |
