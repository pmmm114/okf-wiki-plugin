...
| **원격 반영(PR)** | **`okf-open-pr.sh` + `study.handlers` 배선** | **trust 승인**(`/study --trust`, 머신별) |
...
1. **핸들러 스크립트 + 배선은 원격 main 필수.** 디스패치가 핸들러를 **repo 트리 안 +
   ...
   막혀요. 그래서 핸들러 스크립트(`scripts/okf-open-pr.sh`)와 그 배선
...
참조 골격이 [`examples/okf-open-pr.sh.example`](examples/okf-open-pr.sh.example)에 있어요.
**그대로 쓰는 활성 핸들러가 아니라** 소비처가 자기 커밋 경로(예: `scripts/`)로 복사·
수정하는 뼈대예요. 목적지 repo·리뷰어·라벨은 하드코딩하지 말고 채워 넣어요.

URL vault에서 특히 중요한 건 **격리**예요. 관리형 clone은 유저 스코프의 단일 자원
이라, 핸들러가 그 체크아웃 브랜치를 바꾸거나 미커밋 잔재를 남기면 이후 신선도
갱신이 막혀요. 그래서 핸들러는 **`git worktree`로 임시 워크트리**를 만들어 거기서만
커밋·push해요.
...
| [참조 핸들러 템플릿](examples/okf-open-pr.sh.example) | 계약을 실증하는 PR 핸들러 뼈대(복사·수정용) |
