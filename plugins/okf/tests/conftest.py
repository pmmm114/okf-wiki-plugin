"""플러그인 테스트 공통 — `scripts/` import 경로 추가 + vault 스코프 격리.

`plugins/okf/scripts/`는 패키지가 아니라 훅·스캐폴드 스크립트 모음이므로,
테스트가 모듈로 직접 import할 수 있게 sys.path 선두에 넣는다.

테스트는 "깨끗한 tmp 프로젝트"를 가정하는데, 실행 머신에 vault 포인터
(`~/.claude/okf/vault-project`)나 유저 스코프 런타임(`~/.claude/okf/study`)이
실존하면 okf_vault의 스코프 해소 경유로 그 상태가 새어 들어와 결과가 머신마다
...
_SCOPE_ENV_VARS = (
    "OKF_VAULT_PROJECT",
    "OKF_HOME_PROJECT",  # 구 env — read_pointer의 하위호환 폴백이라 격리도 함께(#152)
    "CLAUDE_CONFIG_DIR",
    "CLAUDE_PROJECT_DIR",
...
def _isolate_vault_scope(tmp_path_factory: pytest.TempPathFactory):
    mp = pytest.MonkeyPatch()
    mp.setenv("HOME", str(tmp_path_factory.mktemp("vault")))
