...
메모리→지식 승격(`study`) 도입은 [docs/adopting-study.md](docs/adopting-study.md) 참조.
...
오라클 차동 → 벤더 동기화 → 라이선스 → 플러그인 검증) 녹색 후 머지합니다.
브랜치·커밋·머지 전략은 [docs/branching.md](docs/branching.md), 배포·버전관리
전략은 [docs/releasing.md](docs/releasing.md), 에이전트 작업 규칙은
