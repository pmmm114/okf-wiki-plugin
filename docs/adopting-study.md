...
| `review` (권장) | 메모리를 저장할 때 후보만 스테이징 저장소(`.okf-study/study.db`)에 쌓아 둡니다. `/study`로 검토해서 승격합니다. 여기에 더해, 턴이 끝날 때 그 턴에 막힌 자리(실패한 명령·거부된 도구)가 있었으면 남길 지식이 있는지 한 번 되물어 봅니다 |
