...
`capture`가 `review` 이상이면 **활용 관측**이 함께 돕니다. 프롬프트와 개념 로드가 로컬 원장에 쌓이고 `study usage-report <project>`가 건수와 개념별 분포를 보여줍니다 — 저장고가 실제로 얼마나 쓰이는지 재는 재료이며, 비율이나 판정은 내지 않습니다. 세션을 몇 번 돌린 뒤 이 집계가 계속 0이라면 관측이 붙지 않은 것이니 `/okf-doctor`로 캡처 상태부터 확인하세요.
