...
- **플러그인 마켓플레이스 채널**은 태그-핀 소비 계층에 합류한다. Claude Code의 버전
  해석 순서는 `plugin.json` → marketplace 엔트리 → 커밋 SHA인데, 이 repo는 `plugin.json`에
  version을 두지 않으므로(불변식) 사람이 읽는 버전은 `.claude-plugin/marketplace.json`의
  okf 엔트리가 진다. 두 단계로 굴린다([`plugin-versioning.md`](plugin-versioning.md)가 정본):
  - **첫 릴리스 전(태그 0개)**: 엔트리에 version 없음 + 상대경로 소스 → 커밋 SHA 추적
    (소비처는 main HEAD를 매 커밋 자동 업데이트받는다).
  - **릴리스 후(태그 ≥1)**: 엔트리를 최신 태그에 핀 — `source`를 git-subdir(자기 참조)로
    바꾸고 `ref=vX.Y.Z` + `version=X.Y.Z`를 동기로 단다. 소비처는 main HEAD가 아니라
    큐레이션된 릴리스를 받고, 다음 컷에서 version이 바뀔 때만 업데이트된다 —
    `actions/validate@vX.Y.Z`·pre-commit `rev`와 같은 태그-핀 계층이다.

  내부 정합(version↔ref)은 `scripts/test_marketplace_version.py`(CI `core` 잡)가 무git으로,
  "최신 태그와 일치"는 `scripts/marketplace_version.py`가 릴리스 때 강제한다 —
...
3. **릴리스 PR** — `okf-core/pyproject.toml`(+루트 셔틀)의 플레이스홀더에 도출된
   **실번호를 기입**, `.claude-plugin/marketplace.json`의 okf 엔트리를 **새 태그로 핀**
   (`python3 scripts/marketplace_version.py X.Y.Z`로 도출 — 첫 릴리스면 Phase P→R 전환),
   `CHANGELOG.md` 갱신, `okf validate .okf --strict`·테스트 재확인. 스쿼시 머지 → 이 커밋이
   릴리스 지점. 자세한 건 [`plugin-versioning.md`](plugin-versioning.md).
...
7. **버전-중립 복귀** — main의 두 pyproject를 **`0.0.0.dev0`으로 되돌리는** 후속
   커밋. 다음 minor를 미리 박지 않는다(#164). ⚠️ **marketplace.json 핀은 되돌리지 않는다** —
   "최신 배포 버전"을 뜻하므로 다음 컷까지 그대로 유지한다(pyproject의 "다음 dev"와 의미가 다름).
   태그 생성 후 `python3 scripts/marketplace_version.py`(무인자)로 최신 태그와 정합을 확인한다.
...
- [ ] `.claude-plugin/marketplace.json` okf 엔트리를 새 태그로 핀(`scripts/marketplace_version.py X.Y.Z` 도출 — git-subdir + `ref=vX.Y.Z` + `version=X.Y.Z` 동기)
...
- [ ] 태그 생성 후 `scripts/marketplace_version.py`(무인자)로 마켓플레이스 핀 == 최신 태그 검증
- [ ] main의 두 pyproject를 **`0.0.0.dev0`으로 복귀**(⚠️ marketplace.json 핀은 유지)
...
- **repo 직배달(D2)** — 소비처는 태그를 핀한다:
  ...
  repo 루트 설치).
- **플러그인 마켓플레이스 채널**도 릴리스 후엔 같은 태그-핀 계층이다 —
  `.claude-plugin/marketplace.json`의 okf 엔트리를 최신 태그에 핀해 소비처가 main HEAD가
  아니라 큐레이션된 릴리스를 받게 한다(첫 릴리스 전엔 SHA 추적). 정본은
  [`plugin-versioning.md`](plugin-versioning.md).
