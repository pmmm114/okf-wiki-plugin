...
- 이 repo 루트 `.okf-wiki.json`은 **자가 사용(주입 전용)** 설정이다 — `.okf/`를
  개발 세션에 주입하되(도그푸딩) `study.capture`는 **`off` 고정**이고 `study.handlers`도
  두지 않는다(목적지 무참조). `.okf/`는 CI `core` 잡의 `okf validate .okf --strict`
  (자기 번들 검증) 게이트 대상이라, study 승격을 이리로 흘리면 큐레이션 번들이 churn·
  스코프 오염되고 strict 실패가 CI를 막는다 — `.okf/`는 **주입 소스이자 CI 타깃**이라
  study 쓰기 대상이 아니다. 개발 지식 캡처가 필요해지면 `.okf/`가 아닌 별도 번들로
  분리해 두 역할이 겹치지 않게 한다.
