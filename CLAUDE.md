...
- `plugins/okf/.claude-plugin/plugin.json`에 **version 필드 금지** — Claude Code 버전
  해석은 plugin.json → marketplace 엔트리 → 커밋 SHA 순이라, plugin.json에 version이 있으면
  marketplace 엔트리를 조용히 덮는다. 그래서 `claude plugin validate`는 비-strict로 실행하고,
  `scripts/test_marketplace_version.py`가 version 부재를 못박는다.
- `.claude-plugin/marketplace.json`의 플러그인 엔트리는 **상대경로 소스(`./…`) + version
  없음**만 허용 — 이 repo는 엔진을 심링크(`plugins/okf/core → ../../okf-core`)로 공유하는
  모노레포라, git 마켓플레이스의 전체-repo 클론(상대경로)만 심링크를 해소한다. **git-subdir
  금지**(하위 디렉터리만 sparse 클론 → 심링크 dangling → 플러그인 깨짐), **정적 version
  금지**(SHA 추적 소스에 박으면 자동 업데이트 동결). 형태 게이트 `test_marketplace_version.py`가
  차단. 릴리스 고정은 소비처가 add 시 `@vX.Y.Z`로 한다 — 정본은 `docs/plugin-versioning.md`.
