...
argument-hint: "[--home <path|url>]"
---

이 repo에 OKF 번들과 study 런타임을 세팅한다. **멱등**하므로 여러 번 실행해도
안전하고, 기존 파일은 덮어쓰지 않는다. 인자: `$ARGUMENTS`.

**`--home <path|url>`가 주어지면 아래 대신 홈 포인터 마법사(#91·#153)를 수행한다.**
홈 값은 **로컬 clone 절대경로** 또는 **repo URL**(ssh/https/git/file) 둘 다 된다.

H1. **검증·기록**: `"${CLAUDE_PLUGIN_ROOT}/bin/okf-py" "${CLAUDE_PLUGIN_ROOT}/scripts/study_scope.py" set <path-or-url>` 실행.
    출력의 `mode`(`"path"`|`"url"`)·`written`으로 분기한다(프롬프트 추측 금지 — 코드 판정).
    - `mode: "path"`, `written: true` → 로컬 경로 홈. `capture_ready` 값으로 H1a 분기 후 H2로.
    - `mode: "url"`, `written: true` → **URL 홈(#153)**. 포인터엔 URL 원문(크레덴셜 제거본)이
      ...
      - `clone_exists: false` → clone이 없다. 사용자에게 `clone_path`와 "생성엔 네트워크·디스크가
        든다"를 알리고 **동의를 받아** `"${CLAUDE_PLUGIN_ROOT}/bin/okf-py" "${CLAUDE_PLUGIN_ROOT}/scripts/okf_remote.py" clone`을
    ...
    - `reason: "대상 없음" | "git repo 아님"`(로컬 경로만) → 사유를 그대로 보이고 종료(경로 오탈자
      또는 git repo가 아닌 대상 — 로컬 홈은 실제 git repo여야 한다).
H1a. **캡처 준비 판정(스크립트 출력 `capture_ready` 기준 — 프롬프트 추측 금지)**:
    ...
    - `"off" | "absent"` → 이 홈은 지금 **주입(읽기) 전용**이라, 다른 위치에서 저장한
      메모리가 이 홈으로 적재되지 않는다.
      - **로컬 경로 홈**: 사용자에게 그 사실과 "위치 무관 적재를 켜려면 홈에 캡처를 활성해야
        한다"를 알리고 **동의를 받아**
        `"${CLAUDE_PLUGIN_ROOT}/bin/okf-py" "${CLAUDE_PLUGIN_ROOT}/scripts/study_scope.py" enable-capture <path>`
        ...
        쓰기이므로 사용자 선택이다).
      - **URL 홈(관리형 clone)**: `enable-capture`는 관리형 clone의 커밋된 설정을 편집해 origin과
