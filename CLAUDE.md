...
- `plugins/okf/.claude-plugin/plugin.json`에 **version 필드 금지** — Claude Code 버전
  ...
  marketplace 핀을 조용히 덮는다. 그래서 `claude plugin validate`는 비-strict로 실행하고,
  `scripts/test_marketplace_version.py`가 version 부재를 못박는다.
- `.claude-plugin/marketplace.json`의 플러그인 엔트리는 **두 형태만 허용** — 첫 릴리스
  전엔 상대경로 + version 없음(SHA 추적, Phase P), 릴리스 후엔 git-subdir 자기참조 핀
  (`ref=vX.Y.Z` + `version=X.Y.Z` **동기**, Phase R). 상대경로 소스에 정적 version을 박으면
  소비처 자동 업데이트가 멈추므로 금지(그렙 아닌 형태 게이트 `test_marketplace_version.py`가
  차단). 버전 도출·검증은 `scripts/marketplace_version.py`, 정본은 `docs/plugin-versioning.md`.
