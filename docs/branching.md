...
> 배포·버전 선정 전략(스코프 마일스톤·커밋-도출 SemVer·컷 절차·체크리스트)은
...
- `.claude-plugin/marketplace.json` 엔트리는 두 형태만 — 상대경로+무version(Phase P) /
  git-subdir 자기참조 핀(Phase R). 상대경로에 정적 version 금지(자동 업데이트 동결, 게이트가 차단).
