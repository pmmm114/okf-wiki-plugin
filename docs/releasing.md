...
- **플러그인 마켓플레이스 채널**은 version 필드 없음(SHA 추적, `plugin.json` 불변식) →
  플러그인의 "버전"은 **소비처가 add 시 핀하는 태그**(`...@vX.Y.Z`)다. 상대경로 소스라 그
  태그의 repo 전체가 클론돼 심링크 엔진까지 해소된다. marketplace.json은 릴리스마다 바꾸지
  않으며, 형태(상대경로·무version)는 `scripts/test_marketplace_version.py`가 CI에서 고정한다.
  정본은 [`plugin-versioning.md`](plugin-versioning.md).
...
3. **릴리스 PR** — `okf-core/pyproject.toml`(+루트 셔틀)의 플레이스홀더에 도출된
   **실번호를 기입**, `CHANGELOG.md` 갱신, `okf validate .okf --strict`·테스트 재확인.
   스쿼시 머지 → 이 커밋이 릴리스 지점. (marketplace.json은 손대지 않는다 — 소비처가
   `@vX.Y.Z`로 핀한다, [`plugin-versioning.md`](plugin-versioning.md).)
...
- [ ] 발행된 Release 본문 확인, 소비 예시 핀 갱신 확인(`actions/validate@vX.Y.Z`, pre-commit `rev`, 플러그인 `@vX.Y.Z`)
...
- **repo 직배달(D2)** — 소비처는 태그를 핀한다:
  ...
  repo 루트 설치).
- **플러그인 마켓플레이스 채널**도 같은 태그-핀 계층이다 — 소비처가 add 시
  `/plugin marketplace add pmmm114/okf-wiki-plugin@vX.Y.Z`로 그 태그의 repo 전체를 받는다
  (핀 없이 add하면 최신 main = SHA 추적). marketplace.json은 릴리스마다 바꾸지 않는다 —
  정본은 [`plugin-versioning.md`](plugin-versioning.md).
