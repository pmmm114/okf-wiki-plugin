...
멱등·비파괴다. 다음을 만들고, 이미 있으면 보존한다:

- `.okf/` — 지식 번들(없을 때만 스캐폴드)
- `.okf-wiki.json` — 프로젝트 설정(있으면 `study` 블록만 보강)
- `.okf-study/` — 런타임 상태 디렉터리 + 자체 `.gitignore`(스테이징은 커밋되지 않음)

### 3. 스모크 테스트 — 번들 검증

```bash
# uv 사용 (권장)
uv run --project okf-core okf validate .okf --strict
...
세션 시작 시 주입을 켜려면 소비 repo 루트에 `.okf-wiki.json`을 둔다:
