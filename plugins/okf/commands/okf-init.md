...
이 repo에 OKF 번들과 study 런타임을 세팅한다. **멱등**하므로 여러 번 실행해도 안전하고, 기존 파일은 덮어쓰지 않는다. 인자: `$ARGUMENTS`.
...
H1. **검증·기록**: `"${CLAUDE_PLUGIN_ROOT}/bin/okf-py" "${CLAUDE_PLUGIN_ROOT}/scripts/study/study_scope.py" set <path-or-url>` 실행. 출력의 `mode`(`"path"`|`"url"`)·`written`으로 분기한다(프롬프트 추측 금지 — 코드 판정).
    ...
    - `written: false, reason: "URL 포인터 — 미지원 transport"` → https/ssh/git/file만 지원함을 알리고 종료(`ext::` 등 명령 실행 transport는 보안상 거부).
    - `reason: ".okf-wiki.json 없음"`(로컬 경로만) → 대상이 아직 소비 repo 골격이 아니다. 사용자 동의를 받아 **그 경로를 cwd로** 일반 초기화(아래 1~2단계)를 수행해 골격을 만들고 `set`을 재시도한다. 스캐폴드 시 `study.capture`는 `review`를 권장 안내.
    - `reason: "대상 없음" | "git repo 아님"`(로컬 경로만) → 사유를 그대로 보이고 종료(경로 오탈자 또는 git repo가 아닌 대상 — 로컬 vault는 실제 git repo여야 한다).
...
3. **결과 요약**: 각 단계 수행/유지 상태를 알리고, `study.capture` 기본값이 `off`(자동 캡처 꺼짐)임을 안내한다. 자동 캡처를 원하면 `review`/`auto`로 올리고, 핸들러는 **git에 커밋된 경로**에 두어야 함을 덧붙인다(설정 상세는 `skills/okf/reference/CONFIG.md`).
